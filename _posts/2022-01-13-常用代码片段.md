...
ushort[] GetCrc8Table()
{
     ...
     * 首先需要明确CRC8常见的表格数据中是按照线传输LSB，并通过右移寄存器来判断的，因此每次要判断的就是寄存器的最低位LSB。
     * 同时要将多项式X^8+X^5+X^4+X^0 （也就是0x131(0011 0001)）按位颠倒后得到0x8c(1000 1100)，
     * 在计算过程中做异或运算（为什么0x131中第一个1没有被颠倒？答：因为它是隐藏的）。
     */
    ushort A = 0x8c;
    ushort[] crc8 = new ushort[256];
...
ushort[] GetCrc16Table()
{
    /*
     *      名称	      生成多项式	    简记式	                        标准引用
     * CRC16-CCITT	x16+x12+x5+1	1021	ISO HDLC, ITU X.25,，V.34/V.41/V.42, PPP-FCS
     * 
     * 多项式X^16+X^12+X^5+X^0 （也就是0x11021(0001 0000 0010 0001)）按位颠倒后得到0x8608(1000 0100 0000 1000)(第一个1没有被颠倒？答：因为它是隐藏的)
     */
    ushort A = 0x8608;
    ushort[] crc16 = new ushort[256];
    Parallel.For(0, 256, i =>
    {
        ushort temp = (ushort)i;
        int index = 0;
