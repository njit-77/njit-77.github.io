...
![1](/images/Csharp中Equal函数与运算符'=='/1.PNG)

### 通用类型系统

![2](/images/Csharp中Equal函数与运算符'=='/2.PNG)

### 值类型Equal函数 and 运算符'=='

#### 常见类型 int、float、double、decimal等虽然继承自ValueType，但其结构体内部重写了Equal。

##### int，float，double，decimal内部的Equal函数和 '=='重载符函数。
...
##### 感兴趣的可去[Reference Source ](http://referencesource.microsoft.com/#mscorlib)查看全部代码。
...
##### 结论：对于简单常见值类型 int、float、double、decimal等，Equal函数 and 运算符'=='，如果其值相等，返回true；否则，返回false。

#### 结构体struct

#### ValueType内部的Equals函数
...
##### 结论：此时程序执行我们重写的Equal函数 and 运算符'=='。

#### 结构体(只有值类型，不重写Equal函数 and 运算符'==')
...
程序执行时，CanCompareBits(this)返回true，代码执行return FastEqualsCheck(thisObj, obj);

##### 结论：程序判断struct里面所有字段的值，如果全部相等，返回true；否则，返回false。

#### 复杂结构体(有值类型、引用类型，重写Equal函数 and 运算符'==')
...
#### 复杂结构体(内部值类型、引用类型，不重写Equal函数 and 运算符'==')
...
程序执行时，CanCompareBits(this)返回false，代码执行ValueType类Equal函数的下面语句
...
##### 结论：程序判断struct里面所有字段,值类型就判断值是否相等；引用类型就判断是否引用相等。

### 引用类型Equal函数 and 运算符'=='

#### 字符串string

C#语言规范5.0中文版中的字符串相等运算符介绍
![3](/images/Csharp中Equal函数与运算符'=='/3.PNG)
string的Equal函数和'=='重载运算符函数代码
...
Object.ReferenceEquals(this, value)如果this、value是同一个引用，返回true；否则，返回false。
...
学习EqualsHelper(String strA, String strB)函数之前，我们先看一段代码
...
![4](/images/Csharp中Equal函数与运算符'=='/4.PNG)

修改后EqualsHelper(String strA, String strB)函数
...
测试EqualsHelper(String strA, String strB)函数
...
结论：string类型 a == b、string.Equals(a, b)、a.Equals(b)、a.Equals((object)b)，如果 a 的值与 b 的值相同，则为 true；否则为 false。

#### 类class

C#语言规范5.0中文版中的引用类型相等运算符介绍
![5](/images/Csharp中Equal函数与运算符'=='/5.PNG)
![6](/images/Csharp中Equal函数与运算符'=='/6.PNG)
...
ReferenceEquals (Object objA, Object objB)返回objA == objB。如果objA、 objB引用同一个对象(只判断是否引用同一个对象，即使我们自行重载了'=='运算符，也没用)，返回true；否则，返回false。
...
参考
[C#类型基础](http://www.cnblogs.com/JimmyZhang/archive/2008/01/31/1059383.html)
C#语言规范5.0中文版
[Reference Source ](http://referencesource.microsoft.com/#mscorlib)
